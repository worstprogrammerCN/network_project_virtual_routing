...
import center_routing.RoutingTable.RoutingDetail;
...
public class Host {
    ...
    private JButton sendNewLinksStateButton; // 界面右下方的"Update!"按钮， 点击后会把自己与其它节点是否相连的情况进行广播 
    private JList<LinkStateItem> linkState; // 用于储存界面右边栏每一行的信息
                                            // 每个{@code LinkStateItem}是一个其它的节点与本节点之间的关系
    private Vector<LinkStateItem> rows;

    // 界面中间的路由信息面板， 包含title和{@code routingMessageField}
    // 显示经过收敛，本节点与 其它节点之间的cost，以及到该节点需要经过的下一跳路由
    private JScrollPane linkStateMiddlePanel; 
    private JScrollPane routingMessageField;  // 附属于{@code linkStateMiddlePanel}, 储存了经过收敛后，本节点与其它节点之间的cost
    private DefaultListModel<String> listModel; // 是{@code routingMessageField}的Model
    ...
    private void sendNewLinksState() {
        ...
        // form is like "UPDATE$dst={@param destIp1}&cost={@param cost1}|dst={@param destIp2}&cost={@param cost2}"
            ...
            while (active) {
                try {
                    ...
                    if ("ADD".equals(mode)) {
                        ...
                        LinkStateItem itemToDelete = null;
    ...
    public void updateRoutingTableField() {
        ...
        for (String dst : dstSet) { // 提取routingTable每一项的信息填充在 listModel里
    ...
    public void updateRoutingRightPanel() {
    	if (routingTable == null) {
            return;
        }
    	rows.removeAllElements();
    	for (String dest : routingTable.keySet()) {
    		...
    		boolean isNeighbour = cost == 1;
    		if (RoutingTable.isInfinity(cost)) {
        		rows.add(new LinkStateItem(isNeighbour, dest, "INF"));
    		} else  {
